...
Tools: `webvector_research` (query, related_queries?, top_k?, max_pages?, freshness?, domains_allow?, domains_block?, session_id?, response_format?, max_tokens?), `webvector_fetch` (url, query?, max_length?, start_index?, include_links?, selector?, exclude_selectors?, response_format?), `webvector_search` (query), `webvector_status`. Progress notifications and `structuredContent` are supported; cancellation is honoured. `--legacy-tool-names` also registers the pre-0.2 names (`web_research`, `web_fetch`, `web_search`) as aliases for one release.
