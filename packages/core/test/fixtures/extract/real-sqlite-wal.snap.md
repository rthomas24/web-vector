...
The [second advantage of WAL-mode](https://www.sqlite.org/wal.html#advantages) is that writers do not block readers and readers do not block writers. This is <u>mostly</u> true. But there are some obscure cases where a query against a WAL-mode database can return [SQLITE\_BUSY](https://www.sqlite.org/rescode.html#busy), so applications should be prepared for that happenstance.
