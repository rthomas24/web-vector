...
| `sqlite` | `WEBVECTOR_SQLITE_STORE` (file path; default `$XDG_DATA_HOME/webvector/store.sqlite` = `~/.local/share/webvector/store.sqlite`) | **Zero-dependency persistent store** on Node's built-in `node:sqlite` (Node ≥ 22.13; falls back to `memory` with a warning where unavailable). Chunks + Float32 BLOB vectors in one file; candidates filtered in SQL (collection / session / URLs), cosine computed in JS — fine up to **~50 000 vectors per collection** (≈ 50 ms/query); use pgvector/Qdrant/Chroma beyond that, or `npm i sqlite-vec` + `store.options.vec: true` to rank inside SQLite (`vec_distance_cosine`). WAL + busy timeout (CLI and MCP server can share the file). `store.mode: persistent` survives restarts: the session's chunk map and BM25 side-index are rebuilt from stored text on first use; session rows expire on disk after `store.sessionTtlMs` (the `persistent` session is retained). `store.collection` separates embedding models. |
